...
The following example creates a single trigger listening to players gaining a new battle rank before printing the character, their title, and the newly gained battle rank.
...
.. note::
   ...
   .. code-block:: python3
      ...
      async with auraxium.EventClient() as client:
         ...
         # When the parser reaches the end of the body, the client just set up
         # the WebSocket connection, but there is nothing that keeps the
         # asynchronous control flow from leaving the block.
